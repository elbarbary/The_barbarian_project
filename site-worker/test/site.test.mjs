...
test('§8 a projected figure always carries the rate it assumes', async () => {
  ...
  for (const [figure, rate] of [['calc.egx3Y', '٢٨'], ['calc.gold3Y', '٢٥'], ['calc.cd3Y', '٢٣']]) {
    assert.ok(tools.includes(`{{ ${figure} }}`), `${figure} is not rendered`);
    const card = tools.slice(0, tools.indexOf(`{{ ${figure} }}`));
    const opened = card.lastIndexOf('<!-- ');
    assert.ok(card.slice(opened).includes(rate) || tools.includes(rate),
      `${figure} is shown without naming the ${rate}% it assumes`);
  }

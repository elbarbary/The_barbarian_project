#!/usr/bin/env python3
"""The exchange's session figures, and the guard they deliberately do not touch.
...
class TickersTest(unittest.TestCase):
    ...
    def test_reads_whole_pounds_keyed_by_ticker(self):
        ...
        rows, written = harvest.fetch()
        self.assertEqual(rows, {"COMI": {"market_cap": 474e9},
                                "ABUK": {"market_cap": 30e9}})
        self.assertIsNone(written)

    def test_the_trade_count_and_the_turnover_come_with_it(self):
        # Both are live on the quotes feed and gone from it at the close, so
        # this copy is what lets a company screen still show them at night.
        harvest.beta.request = lambda path: {"data": {"data": [
            {"reuters": "COMI.CA", "mc": 474e9, "trades": 2137,
             "value": 325701761.4, "writeTime": "202608311253"},
            # A share nobody traded: zero is a fact about the session and is
            # kept, where a missing field is an absence and is not.
            {"reuters": "ABUK.CA", "mc": 30e9, "trades": 0},
        ]}}
        rows, written = harvest.fetch()
        self.assertEqual(rows["COMI"],
                         {"market_cap": 474e9, "trades": 2137, "value": 325701761})
        self.assertEqual(rows["ABUK"], {"market_cap": 30e9, "trades": 0})
        self.assertNotIn("value", rows["ABUK"])
        self.assertEqual(written, "202608311253")
    ...
    def test_refuses_the_millions_column_and_the_junk_rows(self):
        ...
        self.assertEqual(harvest.fetch()[0], {"SWDY": {"market_cap": 12e9}})
...
class DocumentTest(unittest.TestCase):
    def test_the_committed_file_is_whole_pounds_and_plausible(self):
        path = pathlib.Path(__file__).resolve().parent.parent / "data-source" / "egx-beta" / "session.json"
        if not path.exists():
        ...
        self.assertTrue(all(v > 0 for v in caps))
        # Turnover is pounds and volume is shares; a turnover smaller than the
        # trade count would mean the two had been swapped.
        for ticker, row in rows.items():
            if row.get("trades") and row.get("value"):
                self.assertGreater(row["value"], row["trades"], ticker)

...
class ADelistedShareIsNotAListing(Harness):
    """The scan's list of shares is not the exchange's list of listings.

    The vendor kept quoting Nile Cotton Ginning for five years after the
    exchange delisted it (NewsID 211501, June 2021), so the directory published
    it at a 2.65bn market value beside the companies that really trade.
    """
    ...
    def with_ncgc(self, as_of="2026-09-03T11:00:00.000Z"):
        body = scan(records=3, as_of=as_of)
        body["records"].append({"ticker": "NCGC", "company": "Nile Cotton Ginning",
                                "close": 50.0, "change": 0.0, "volume": 800,
                                "marketCap": 2649625015, "sector": "Process Industries"})
        body["scannerTotal"] = len(body["records"])
        return body

    def build_with(self, body, quotes_only=False, delisted=None):
        path = pathlib.Path(self.tmp.name) / "scan.json"
        path.write_text(json.dumps(body), encoding="utf-8")
        return bma.build(path, True, quotes_only=quotes_only, delisted=delisted)

    def read(self, name):
        return json.loads((self.api / name).read_text(encoding="utf-8"))

    def test_no_row_no_document_no_quote_and_named_with_its_notice(self):
        self.assertEqual(self.build_with(self.with_ncgc(), delisted={"NCGC": self.NCGC}), 0)
        directory = self.read("companies.json")
        self.assertNotIn("NCGC", [c["ticker"] for c in directory["companies"]])
        self.assertEqual(len(directory["companies"]), 3)
        self.assertEqual(directory["delisted"], [{
            "ticker": "NCGC", "name_en": "Nile Cotton Ginning", "name_ar": bma.ARABIC.get("NCGC"),
            "isin": "EGS32131C012", "delisted_on": "2021-06-14", "news_id": 211501,
            "link": "https://www.egx.com.eg/en/NewsDetails.aspx?NewsID=211501",
            "kind": "voluntary"}])
        self.assertFalse((self.api / "companies" / "NCGC.json").exists())
        self.assertFalse((self.fixtures / "companies" / "NCGC.json").exists())
        self.assertNotIn("NCGC", self.read("market.json")["stocks"])

    def test_a_listed_share_is_untouched(self):
        self.assertEqual(self.build_with(self.with_ncgc(), delisted={}), 0)
        directory = self.read("companies.json")
        self.assertIn("NCGC", [c["ticker"] for c in directory["companies"]])
        self.assertEqual(directory["delisted"], [])

    def test_the_default_asks_the_exchange_notices(self):
        with mock.patch.object(bma.listing_status, "delisted", lambda: {"NCGC": self.NCGC}):
            self.assertEqual(self.build_with(self.with_ncgc()), 0)
        self.assertNotIn("NCGC", self.read("market.json")["stocks"])

    def test_the_fast_path_keeps_the_quote_until_the_directory_drops_the_row(self):
        # A notice harvested after the last full build: the directory still
        # lists the share, so the quote stays beside it rather than leaving a
        # row with no price.
        self.assertEqual(self.build_with(self.with_ncgc(), delisted={}), 0)
        self.assertEqual(
            self.build_with(self.with_ncgc("2026-09-03T12:00:00.000Z"), quotes_only=True,
                            delisted={"NCGC": self.NCGC}), 0)
        self.assertIn("NCGC", self.read("market.json")["stocks"])
        # The full build takes the row out; from then on the fast path agrees.
        self.assertEqual(
            self.build_with(self.with_ncgc("2026-09-03T13:00:00.000Z"),
                            delisted={"NCGC": self.NCGC}), 0)
        self.assertEqual(
            self.build_with(self.with_ncgc("2026-09-03T14:00:00.000Z"), quotes_only=True,
                            delisted={"NCGC": self.NCGC}), 0)
        self.assertNotIn("NCGC", self.read("market.json")["stocks"])

    def test_an_unreadable_notice_archive_costs_the_exclusion_not_the_prices(self):
        def broken():
            raise RuntimeError("archive unreadable")
        with mock.patch.object(bma.listing_status, "delisted", broken):
            self.assertEqual(self.build_with(self.with_ncgc(), quotes_only=True), 0)
        self.assertIn("NCGC", self.read("market.json")["stocks"])
